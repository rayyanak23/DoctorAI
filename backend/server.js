...
// Start-session endpoint to provide a greeting
app.post('/start-session', async (req, res) => {
    // Safer access to req.body using optional chaining (?.)
...
// Generate followup form based on symptoms
    ...
    db.run(
        ...
        function (err) {
            ...
            for (const [question, answer] of Object.entries(responses)) {
                const safeAnswer = (answer || 'No answer provided').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/&/g, '&amp;');
